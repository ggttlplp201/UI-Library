...
function build(): Page[] {
  const page: Page = {
    ...
    instances: [
      ...
      // Image hover studies.
      inst(preset('src/hover/ClipReveal.tsx'), 140, 1420, {}),
      inst(preset('src/hover/HoverExpand.tsx'), 640, 1420, {}),
      // Rolling band between image blocks.
      inst(preset('src/effects/Marquee.tsx'), 0, 1900, {
        w: 1280,
        args: { children: 'FIELD NOTES · STUDIES · PROCESS · FIELD NOTES · STUDIES · PROCESS ·' },
      }),
      // Second poster row + social close.
      inst(preset('src/carousels/Carousels.tsx', 'Carousel_001'), 0, 2020, { w: 1280 }),
      inst(preset('src/hover/SocialHoverGrid.tsx'), 990, 2480, {}),
      inst(preset('src/sections/SiteFooter.tsx'), 0, 2620, {
        w: 1280,
