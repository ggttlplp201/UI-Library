...
function build(): Page[] {
  const page: Page = {
    ...
    instances: [
      ...
      inst(preset('src/cards/SpotlightCard.tsx'), 140, 780, {
        w: 330,
        h: 330,
        args: { children: 'Texture study — powdered graphite on cotton rag. Move the light across it.' },
        anim: { preset: 'slide-up', trigger: 'scroll', duration: 0.6, delay: 0, easing: 'ease-out', once: true },
      }),
      inst(preset('src/cards/HoverBlurCards.tsx'), 520, 780, {
        anim: { preset: 'slide-up', trigger: 'scroll', duration: 0.6, delay: 0.1, easing: 'ease-out', once: true },
      }),
      inst(preset('src/cards/GlowFlipCard.tsx'), 930, 780, {
        anim: { preset: 'slide-up', trigger: 'scroll', duration: 0.6, delay: 0.2, easing: 'ease-out', once: true },
      }),
      // Image hover studies.
      inst(preset('src/basics/Heading.tsx'), 140, 1220, { args: { text: 'Studies', size: 28 } }),
      inst(preset('src/basics/Divider.tsx'), 140, 1270, { args: { width: 1000 } }),
      inst(preset('src/hover/ClipReveal.tsx', 'ClipDiv'), 140, 1320, {}),
      inst(preset('src/hover/HoverExpand.tsx', 'HoverExpand_001'), 860, 1320, {}),
      ...
      inst(preset('src/effects/Marquee.tsx'), 0, 2220, {
        w: 1280,
        args: { children: 'FIELD NOTES · STUDIES · PROCESS · FIELD NOTES · STUDIES · PROCESS ·' },
      }),
      // Second poster row + social close.
      inst(preset('src/carousels/Carousels.tsx', 'Carousel_001'), 0, 2330, { w: 1280 }),
      inst(preset('src/hover/SocialHoverGrid.tsx'), 528, 2760, {}),
