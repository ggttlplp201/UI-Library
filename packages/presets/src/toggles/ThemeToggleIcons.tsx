...
/**
 * Animated theme-toggle icon buttons (sun/moon morphs).
 * Ported from Skiper UI (skiper4), itself adapted from https://toggles.dev/
 * (Open Source CSS Theme Toggles by Alfie Jones), rebuilt with Framer Motion.
 * Attribution to Skiper UI is required when using the free version.
 * Author: @gurvinder-singh02 (https://gxuri.me)
 */

/**
 * Showcase row of all five toggles, keeping the original demo's
 * arrangement options (scale / gap / direction) as props.
 */
export const ThemeToggleIconsShowcase = ({
  /** Scale of the whole row, 0–100 (0 = natural size) */
  scale = 0,
  /** Gap between the icons, px */
  gap = 0,
  /** Stack the icons in a row or a column */
  direction = "row",
}: {
  scale?: number;
  gap?: number;
  ...
  return (
    <motion.div
      className="relative flex items-center justify-center gap-1 p-6"
      animate={{
        gap: gap ? `${gap}px` : "4px",
...
export const ThemeToggleButton1 = ({
  ...
  return (
    <button
      ...
      data-hover-demo="click"
      className={cn(
        "size-12 rounded-full bg-black text-white transition-all duration-300 active:scale-95",
        className,
      )}
...
export const ThemeToggleButton2 = ({
  ...
  return (
    <button
      ...
      data-hover-demo="click"
      className={cn(
        "size-12 rounded-full border border-border p-2 transition-all duration-300 active:scale-95",
...
export const ThemeToggleButton5 = ({
  ...
  return (
    <button
      ...
      data-hover-demo="click"
      className={cn(
        "size-12 rounded-full border border-border p-3 transition-all duration-300 active:scale-95",
        isDark ? "bg-black text-white" : "bg-white text-black",
        className,
      )}
      onClick={() => setIsDark(!isDark)}
