...
/**
 * SettingsNavList — a GitHub-style settings rail: hovering one row blurs and
 * shrinks its siblings; the active row carries a blue focus bar.
 * Ported from UIverse.io (nervous-treefrog-70) by HetPatel69.
 * Source: https://uiverse.io/HetPatel69/nervous-treefrog-70
 * License: MIT. Attribution: HetPatel69 via UIverse.io.
 */
...
const ICON_ORDER = ["profile", "gear", "brush", "access", "bell"];

export const SettingsNavList = ({
  items = "Public profile, Account, Appearance, Accessibility, Notifications",
  accent = "#2F81F7",
  background = "#0D1117",
}: {
  /** Comma-separated row labels (up to 5 get icons) */
  items?: string;
  /** Active-row indicator color */
  accent?: string;
  /** Panel background */
  background?: string;
}) => {
    ...
    <>
      <style>{`
        .uv-snl-input {
          ...
          width: 200px;
          background-color: var(--uv-snl-bg);
        ...
        .uv-snl-value:not(:active):hover, .uv-snl-value:focus { background-color: #21262C; }
        .uv-snl-value:focus, .uv-snl-value:active, .uv-snl-value.uv-snl-on { background-color: #1A1F24; outline: none; }
        .uv-snl-value::before {
          content: "";
          position: absolute;
          top: 5px;
          ...
          width: 5px;
          height: 80%;
          background-color: var(--uv-snl-accent);
          border-radius: 5px;
          opacity: 0;
        }
        .uv-snl-value:focus::before, .uv-snl-value:active::before, .uv-snl-value.uv-snl-on::before { opacity: 1; }
        .uv-snl-value svg { width: 15px; }
        .uv-snl-input:hover > :not(.uv-snl-value:hover) {
          transition: 300ms;
          filter: blur(1px);
      ...
      `}</style>
      <div
        className="uv-snl-input"
        style={{ "--uv-snl-bg": background, "--uv-snl-accent": accent } as React.CSSProperties}
      >
        {labels.map((label, i) => (
          <button
            key={`${label}-${i}`}
            ...
            className={`uv-snl-value${i === active ? " uv-snl-on" : ""}`}
            onClick={() => setActive(i)}
          >
            {ICONS[ICON_ORDER[i % ICON_ORDER.length]]}
            {label}
          </button>
        ))}
      </div>
