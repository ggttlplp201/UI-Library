/**
 * IconSlideButton — icon-block button whose accent color sweeps across the
 * whole surface on hover, inverting the label.
 * Ported from UIverse.io (pretty-grasshopper-57) by Creatlydev.
 * Source: https://uiverse.io/Creatlydev/pretty-grasshopper-57
 * License: MIT. Attribution: Creatlydev via UIverse.io.
 */
import { useId, type CSSProperties } from "react";

export const IconSlideButton = ({
  label = "Payments",
  accent = "#00ad54",
}: {
  /** Button text */
  label?: string;
  /** Accent color that sweeps in on hover */
  accent?: string;
}) => {
  const uid = useId().replace(/[^a-zA-Z0-9]/g, "");
  const gradId = `uv-isb-grad-${uid}`;
  const maskId = `uv-isb-mask-${uid}`;
  return (
    <>
      <style>{`
        .uv-isb-button {
        ...
        .uv-isb-decor {
          position: absolute;
          inset: 0;
          background-color: var(--uv-isb-accent);
        ...
        .uv-isb-button:hover .uv-isb-text {
          color: #fff;
        }
        .uv-isb-button:hover .uv-isb-decor {
          transform: translate(0);
        }
      `}</style>
      <button
        type="button"
        className="uv-isb-button"
        style={{ "--uv-isb-accent": accent } as CSSProperties}
      >
        <span className="uv-isb-decor"></span>
        <div className="uv-isb-content">
          <div className="uv-isb-icon">
          ...
          <span className="uv-isb-text">{label}</span>
        </div>
      </button>
    </>
  );
